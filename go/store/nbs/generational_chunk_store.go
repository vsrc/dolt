...
func (gcs *GenerationalNBS) Put(ctx context.Context, c chunks.Chunk, getAddrs chunks.GetAddrsCb) error {
	return gcs.newGen.putWithRefCheck(ctx, c, getAddrs, gcs.hasMany)
}
...
func (gcs *GenerationalNBS) PruneTableFiles(ctx context.Context) error {
	err := gcs.oldGen.pruneTableFilesWithRefCheck(ctx, gcs.hasMany)
	if err != nil {
		return err
	}

	return gcs.newGen.pruneTableFilesWithRefCheck(ctx, gcs.hasMany)
}
...
func (gcs *GenerationalNBS) SetRootChunk(ctx context.Context, root, previous hash.Hash) error {
	return gcs.newGen.setRootChunkWithRefCheck(ctx, root, previous, gcs.hasMany)
}
