...
func OverwriteStoreManifest(ctx context.Context, store *NomsBlockStore, root hash.Hash, tableFiles map[hash.Hash]uint32, appendixTableFiles map[hash.Hash]uint32) (err error) {
	...
	panic("unexpected call to OverwriteStoreManifest")
...
func (nbs *NomsBlockStore) Put(ctx context.Context, c chunks.Chunk, getAddrs chunks.GetAddrsCb) error {
	return nbs.putWithRefCheck(ctx, c, getAddrs, nbs.hasMany)
}

func (nbs *NomsBlockStore) putWithRefCheck(ctx context.Context, c chunks.Chunk, getAddrs chunks.GetAddrsCb, checker refCheck) error {
	t1 := time.Now()
...
func (nbs *NomsBlockStore) addChunk(ctx context.Context, ch chunks.Chunk, addrs hash.HashSet, checker refCheck) (bool, error) {
	...
	if !ok {
		...
		if err != nil {
			if errors.Is(err, ErrDanglingReferences) {
				nbs.mt = nil
			}
			return false, err
		}
...
func (nbs *NomsBlockStore) errorIfDangling(head hash.Hash, checker refCheck) error {
	var err error
	...
	if nbs.mt != nil && nbs.mt.pendingRefs != nil {
		...
		if err != nil {
			...
			return fmt.Errorf("found dangling references to memtable pendingRefs %s\n%s", absent.String(), string(debug.Stack()))
		}
	}
	var headHR [1]hasRecord
	a := addr(head)
	headHR[0].a = &a
	headHR[0].prefix = a.Prefix()
	absent, err = checker(headHR[:])
	if err != nil {
		return err
	} else if absent.Size() > 0 {
		return fmt.Errorf("found dangling references to memtable pendingRefs %s", absent.String())
	}
	return nil
}
...
func (nbs *NomsBlockStore) commit(ctx context.Context, current, last hash.Hash, checker refCheck) (success bool, err error) {
	...
	if err = nbs.errorIfDangling(current, checker); err != nil {
		nbs.mt = nil
		for _, n := range nbs.tables.novel {
			n.close()
		}
		nbs.tables.novel = nil
		return false, err
	}
...
func (nbs *NomsBlockStore) PruneTableFiles(ctx context.Context) (err error) {
	return nbs.pruneTableFilesWithRefCheck(ctx, nbs.hasMany)
}

func (nbs *NomsBlockStore) pruneTableFilesWithRefCheck(ctx context.Context, checker refCheck) (err error) {
	nbs.mu.Lock()
	defer nbs.mu.Unlock()
	nbs.waitForGC()
...
func (nbs *NomsBlockStore) MarkAndSweepChunks(ctx context.Context, last hash.Hash, keepChunks <-chan []hash.Hash, dest chunks.ChunkStore) error {
	...
	if r != last {
		panic("root changed during MarkAndSweepChunks")
	}
...
func (nbs *NomsBlockStore) swapTables(ctx context.Context, specs []tableSpec) (err error) {
	...
	tc, err := nbs.tables.rebase(ctx, upstream.specs, nbs.stats)
	if err != nil {
		return err
	}
	tc.novel = nbs.tables.novel
	nbs.tables = tc
...
func (nbs *NomsBlockStore) SetRootChunk(ctx context.Context, root, previous hash.Hash) error {
	return nbs.setRootChunkWithRefCheck(ctx, root, previous, nbs.hasMany)
}

func (nbs *NomsBlockStore) setRootChunkWithRefCheck(ctx context.Context, root, previous hash.Hash, checker refCheck) error {
	nbs.mu.Lock()
	defer nbs.mu.Unlock()
	nbs.waitForGC()
	for {
